...
const defaults = {
  ...
  network: {
    networkId: {
      group: 'network',
      type: 'string',
      description: 'Network ID to run the command against. Defaults to ID assoicated with --network as per truffle config.',
      example: '1',
    },
    network: {
      group: 'network',
      ...
      if ('function' === type) {
        ...
        commands[name] = merge({
          ...
          handler: async (argv) => {
            ...
            if (!this.provider) {
              throw new Error('No Web3 Provider configured.');
            }
