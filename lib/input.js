...
module.exports = function (targetType, options = {}) {
  const transformers = merge(defaults, options.input);
  const transformersForSourceTypes = transformers[targetType];
  if (!transformersForSourceTypes) {
    throw new Error(`Unsupported input type: ${targetType}`);
  }

  let fn;
  // handle special case where we assume the alias for these to override
  // all length specific versions of the same type
  if (targetType.match('bytes') && options.input.bytes) {
    fn = options.input.bytes;
  } else if (targetType.match('int') && options.input.int) {
    fn = options.input.int;
  } else if (targetType.match('uint') && options.input.uint) {
    fn = options.input.uint;
  }
  ...
  return function (value) {
    ...
    fn = transformersForSourceTypes[inputType];
    if (!fn) {
      throw new Error(`Unmapped input value type ${inputType} for solidity taget type ${targetType}`);
    }
    return fn(value);
  };
};
